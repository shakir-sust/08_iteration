...
```{r density plot} 
nematode %>%
  pivot_longer(cols = mel_s:re) %>% #to create 'long' form from wide 'form'
  arrange(name) %>%
  ggplot(aes(x = value)) +
  geom_density() +
...
#Manual process
nematode %>%
  mutate(rndvi = (nir - r)/(nir + r)) %>% #rndvi = ndvi based on red
...
```
# Important for Assignment on Iteration
...
meancv2(df = nematode_w, #remains same
        col = sum_s #changes based on what we want to get
        )
...
nematode_w
ncol(nematode_w) #number of columns in 'nematode_w' data frame
