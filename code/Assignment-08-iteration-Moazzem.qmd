...
myplot <- function(df, x, y) {
  ggplot(df, aes(x = x, 
    ...
    labs(title = "Boxplot of K leaf concentration as a function of K rate",
         x = "K rate (kgha)",
...
# Get the unique locations from your data
locations <- unique(soyk_w$location)

# Iterate over each location and create a plot
plots_list <- map(locations, function(loc) {
  # Filter the data for the current location
  df_loc <- soyk_w %>% dplyr::filter(location == loc)
  
  # Create the plot using the myplot function and add a title with the location
...
# Display the list of plots (each plot corresponds to a location)
plots_list
